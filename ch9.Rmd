...
```{r}
set.seed(10111)
# create a 20x2 matrix with each value randomly sampled from a normal distribution with mean 0 and sd 1
x=matrix(rnorm(40),20,2)
# create a vector of size 20, with -1 occurring 10 times and 1 occuring 10 times
y=rep(c(-1,1),c(10,10))
# add 1 to the x values where y = 1
x[y==1,]=x[y==1,]+1
...
Recall that support vectors are the points that are close to the boundary or on the wrong side of the boundary. In this case there are 6 of them.
...
We see in this case that the radial kernel has done an excellent job. The blue line represents the true decision boundary, and the boundary estimated from the data tracks pretty well with the true boundary.